macro_rules! include_content_str {
    ($file:expr $(,)?) => {
        include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/content/", $file))
    };
}
...
pub(crate) use {include_assets_font, include_content_image, include_content_str};
