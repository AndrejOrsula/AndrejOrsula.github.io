...
pub struct ScrollableFramedCentralPanel {
    ...
    #[builder(default = egui::Margin {left: 0.0, right: 8.0, top: 0.0, bottom: 0.0})]
...
impl ScrollableFramedCentralPanel {
    pub fn show<R>(
        ...
        debug_assert!(self.max_content_width.is_sign_positive());
        debug_assert!(self.min_inner_margin.left.is_sign_positive());
        debug_assert!(self.min_inner_margin.right.is_sign_positive());
        debug_assert!(self.min_inner_margin.top.is_sign_positive());
        debug_assert!(self.min_inner_margin.bottom.is_sign_positive());

        egui::CentralPanel::default().show(ctx, |ui| {
            egui::ScrollArea::vertical()
                .show(ui, |ui| {
                    let margin_x =
                        (ctx.screen_rect().width() - self.max_content_width).max(0.0) / 2.0;
                    let inner_margin = egui::Margin {
                        left: margin_x.max(self.min_inner_margin.left),
                        right: margin_x.max(self.min_inner_margin.right),
...
pub fn clickable_url(response: egui::Response, url: impl ToString) -> egui::Response {
    debug_assert!(response.sense.click);
